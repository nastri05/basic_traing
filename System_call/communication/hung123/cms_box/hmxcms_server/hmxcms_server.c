...
int cms_full_queue_handler(cms_msg_t *msg, mqd_t mqdes, char *client_name)
{
    ...
    if ((msg->tag == CMS_RESPONSE_MESSAGE)) {
        ...
        //check client crash
        if ((pl->send_count >= max_msg) && ((strcmp(pl->source, oldest_cms_msg->source) == 0) && (strcmp(pl->topic, oldest_cms_msg->source) == 0) && (strcmp(pl->data, oldest_cms_msg->data)== 0))) {
            //client crashed
        ...
        //printf(" %s %s %s %s %s %s\n", pl->source, msg->topic, pl->topic, msg->source, pl->data, msg->data);
        if (pl == NULL) {
            //store message in crash list 
        ...
        //check client crash
        //printf("Oldest message %s %s %s\n", oldest_cms_msg->topic, oldest_cms_msg->source, oldest_cms_msg->data);
        if ((pl != NULL) && (pl->send_count >= max_msg) && ((strcmp(pl->source, oldest_cms_msg->source) == 0) && (strcmp(pl->topic, oldest_cms_msg->topic) == 0) && (strcmp(pl->data, oldest_cms_msg->data) == 0))) {
            //client crashed
            LOG_SERVER_STATE("Client is crashed\n");
            char name_crash[64];
...
void cms_server_start(mqd_t mqdes)
{
    ...
    while (1)
    {
        ...
        if (rcv == CMS_ERROR) {
            ...
            // LOG_SERVER_INT(rcv);
            // LOG_SERVER_INT(errno);
            printf("Message nhan thu %d: %s %s %s\n", i , cms_msg_recv.source, cms_msg_recv.topic, cms_msg_recv.data);
...
int main () 
{
    ...
    mqd = cms_server_init();
    if (mqd == CMS_ERROR) {
        LOG_SERVER_STATE("Init server failed \n");
        exit(EXIT_FAILURE);
    }
