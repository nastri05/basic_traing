...
int cms_full_queue_handler(cms_msg_t *msg, mqd_t mqdes, char *client_name)
{
    ...
    if ((msg->tag == CMS_SUBCRIBE_MESSAGE) || (msg->tag == CMS_UNSUBCRIBE_MESSAGE)) {
        ...
        //check client crash
        if ((pl->send_count >= max_msg) && ((strcmp(pl->source, oldest_cms_msg->topic) + strcmp(pl->destination, oldest_cms_msg->source) + strcmp(pl->data, oldest_cms_msg->data)) == 0)) {
            //client crashed
        ...
        if ((pl != NULL) && (pl->send_count >= max_msg) && ((strcmp(pl->source, oldest_cms_msg->source) + strcmp(pl->destination, oldest_cms_msg->topic) + strcmp(pl->data, oldest_cms_msg->data)) == 0)) {
            //client crashed
            LOG_SERVER_STATE("Client is crashed\n");
            char name_crash[64];
