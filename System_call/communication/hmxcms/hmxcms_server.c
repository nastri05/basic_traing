...
int create_handling_pthread()//cms_msg_t *msg)
{
    pthread_t thread_id;
    int ret;
    ret = pthread_create(&thread_id, NULL, &msg_handler, NULL);
    if (ret) {
...
mqd_t cms_server_init()
{
    ...
    if(set_mq_attr(&attr, FLAG, MAX_MSG, MSG_SIZE) == CMS_ERROR) {
...
{   
    ...
    {
        sleep(1);
        int read = cms_receive(mqd, &msg);
        if (read < 0) {
            printf(".");
            fflush(stdout);
        } else {
            //printf("1\n");
            printf("\n gia tri %d %s\n", msg.tag, msg.payload.client_name);
            // if (create_handling_pthread() == CMS_ERROR)
            // {
